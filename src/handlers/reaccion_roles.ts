    ...
    try {
      ...
      for (let i = 0; i < reaccionroles.length; i++) {
        ...
        if (reaction.message.id === reaccionroles[i].ID_MENSAJE) {
          const usuario: GuildMember | undefined = reaction.message.guild.members.cache.get(user.id);
          const parametros = reaccionroles[i].Parametros;
          ...
          for (let k = 0; k < parametros.length; k++) {
            ...
            if (reaction.emoji?.id == parametros[k].Emoji) {
              try {
                ...
                const role: Role | undefined = reaction.message.guild.roles.cache.get(parametros[k].Rol);
                ...
                if (role && usuario) {
                  ...
                  if (usuario.guild.members.me && usuario.guild.members.me.roles.highest.rawPosition > role.position) {
                    await usuario.roles.add(rol).catch((e) => { e });
                    //Si no, entonces hacemos return respondiendo que el bot no tiene permisos
                  } else {
                    (reaction.message.channel as TextChannel).send(`❌ **No tengo suficientes permisos para dar el rol!**\nAsegúrate de mi rol esté por encima del rol a dar!`).catch(() => { }).then((msg) => {
                      if (msg) {
                        setTimeout(() => {
                          msg.delete()
                        }, 3000);
                  ...
                  (reaction.message.channel as TextChannel).send(`❌ **Ese rol ha sido eliminado!**`).catch(() => { }).then((msg) => {
                    if (msg) {
                      setTimeout(() => {
                        msg.delete()
                      }, 3000);
              ...
              try {
                ...
                if (role && usuario) {
                  //Si el rol a dar está por debajo de los roles del bot, le damos el rol
                  if (usuario.guild.members.me && usuario.guild.members.me.roles.highest.position > role.position) {
                    await usuario.roles.add(rol).catch((e) => { e });
                    //Si no, entonces hacemos return respondiendo que el bot no tiene permisos
                  } else {
                    (reaction.message.channel as TextChannel).send(`❌ **No tengo suficientes permisos para dar el rol!**\nAsegúrate de mi rol esté por encima del rol a dar!`).catch(() => { }).then((msg) => {
